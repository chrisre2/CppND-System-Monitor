...
long LinuxParser::ActiveJiffies() {
  ...
  std::transform(jiffies.begin(), jiffies.end(),
                 std::back_inserter(jiffiesLong),
                 [&](std::string s) { return std::stol(s); });
  return jiffiesLong[0] + jiffiesLong[1] + jiffiesLong[2] + jiffiesLong[5] +
         jiffiesLong[6] +
         jiffiesLong[7];  // user and nice had already the guest values
}
...
vector<string> LinuxParser::CpuUtilization() {
  ...
  vector<string> jiffies{
      10};  // 0=user, 1=nice, 2=system, 3=idle, 4=iowait, 5=irq, 6=softirq,
            // 7=steal, 8=guest, 9=guest_nice
  if (stream.is_open()) {
    std::getline(stream, line);
    std::istringstream linestream(line);
    linestream >> key >> jiffies[0] >> jiffies[1] >> jiffies[2] >> jiffies[3] >>
        jiffies[4] >> jiffies[5] >> jiffies[6] >> jiffies[7] >> jiffies[8] >>
        jiffies[9];
  }
  return jiffies;
}
